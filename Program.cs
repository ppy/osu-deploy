...
namespace osu.Desktop.Deploy
{
    internal static class Program
    {
        ...
        private static string stagingPath => Path.Combine(currentSolutionPath, staging_folder);
        private static string templatesPath => Path.Combine(currentSolutionPath, templates_folder);
        private static string releasesPath => Path.Combine(currentSolutionPath, releases_folder);
            ...
            switch (targetPlatform)
            {
                ...
                case RuntimeInfo.Platform.Linux:
                    ...
                    runCommand(appImageToolPath,
                        $"\"{stagingTarget}\" -u \"gh-releases-zsync|ppy|osu|latest|osu.AppImage.zsync\" \"{Path.Combine(currentSolutionPath, "releases")}/osu.AppImage\" --sign", false);
        ...
        private static void buildForMac(string arch, string version)
        {
            ...
            if (!string.IsNullOrEmpty(CodeSigningCertificate))
            {
                ...
                runCommand("codesign",
                    $"--deep --force --verify --entitlements {Path.Combine(currentSolutionPath, "osu.entitlements")} -o runtime --verbose --sign \"{CodeSigningCertificate}\" {stagingApp}");
        ...
        private static bool runCommand(string command, string args, bool useSolutionPath = true)
        {
            ...
            {
                WorkingDirectory = useSolutionPath ? solutionPath : currentSolutionPath,
                CreateNoWindow = true,
